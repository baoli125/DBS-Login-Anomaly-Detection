...
"""
Rule-Based Phát hiện Evaluation - DEMO VERSION

Script này evaluates the dựa trên quy tắc phát hiện system on a tập dữ liệu of login events.
It is designed for demo and presentation:
    1. Loads events from NDJSON file (generated by run_generator.py).
    2. Runs dựa trên quy tắc phát hiện and prints detailed metrics (precision, recall, F1, etc.).
    3. Saves reports for later comparison with ML results.
    4. Prints recommendations for improving rules.
"""
...
def parse_timestamp(timestamp):
    """Phân tích timestamp from various formats (compatible with new aggregator)"""
...
def evaluate_events(events, no_cooldown=False, verbose=False):
    """
    Evaluate events using the new dựa trên quy tắc system
...
def main():
    """
    Chính entrypoint for the dựa trên quy tắc phát hiện demo step.
    1. Loads events from NDJSON file (output of run_generator.py).
    2. Runs dựa trên quy tắc phát hiện and prints detailed metrics (precision, recall, F1, etc.).
    3. Saves reports for later comparison with ML results.
    4. Prints recommendations for improving rules.
    """
    ...
    # Step 1: Validate tập dữ liệu file
    if not os.path.exists(args.dataset):
        print(f" Dataset not found: {args.dataset}")
        sys.exit(1)
    
    # Step 2: Tải events (quiet)
    events = load_ndjson(args.dataset, max_events=args.max_events)
    if not events:
        print(" No events loaded. Exiting.")
        sys.exit(1)
    
    # Step 3: Set custom rules directory if specified
    rule_loader_args = {}
    if args.rules_dir:
    ...
    try:
        ...
        # Step 5: Print results (this is the main output we want to show)
        print_results(results, detailed=args.verbose)
        
        # Step 6: Save reports (quiet)
        save_report(alerts, results, args.output_dir)
        
        # Step 7: Print recommendations (quiet)
        if not args.verbose:
            # Only show recommendations if not verbose
            print("\n RECOMMENDATIONS:")
            print("-"*40)
