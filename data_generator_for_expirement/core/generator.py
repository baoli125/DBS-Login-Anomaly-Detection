...
"""
Chính data generator - SIMPLE VERSION (Fixed for design principles)
"""
...
class SimpleDataGenerator:
    ...
    # 2. Thêm hàm generate_attack_with_fixed_ip() cho rapid tấn công
    def generate_rapid_attack_fixed(self, start_time, duration_seconds=30):
        """Tạo rapid tấn công ĐÚNG NGHĨA: 1 IP → 1 USER → 400 attempts trong 30s"""
        events = []
    ...
    def generate_normal_traffic(self, start_time, end_time, events_per_hour=50):
        """Giai đoạn 1: xây dựng lịch, sắp xếp theo thời gian, sau đó gán IP cố định tuần tự (tối đa 1 IP/user/10 phút)."""
    ...
    def generate_weekend_traffic(self,
        ...
        """Tạo traffic cuối tuần (lưu lượng thấp hơn, tỉ lệ thành công cao hơn)"""
        events = []
        duration_hours = (end_time - start_time).total_seconds() / 3600
    ...
    def generate_distributed_burst_attack(self, start_time, end_time):
        """Generate distributed tấn công - FIXED VERSION"""
        events = []
        target_user = random.choice(self.users)['username']
    ...
    def generate_targeted_slow_low_attack(self, start_time, end_time):
        """Generate targeted slow-low tấn công pattern"""
        events = []
        duration_hours = (end_time - start_time).total_seconds() / 3600
